...
// static analysis
DOCTEST_MSVC_SUPPRESS_WARNING(
    26439) // This kind of function may not throw. Declare it 'noexcept'
...
// - exception specification does not match previous declaration 4350 - behavior
// change: 'member1' called instead of 'member2' 4668 - 'x' is not defined as a
// preprocessor macro, replacing with '0' for '#if/#elif' 4365 - conversion from
// 'int' to 'unsigned long', signed/unsigned mismatch 4774 - format string
// expected in argument 'x' is not a string literal 4820 - padding in structs
...
namespace doctest
{
    ...
    // A 24 byte string class (can be as small as 17 for x64 and 13 for x86)
    // that can hold strings with length of up to 23 chars on the stack before
    // going on the heap - the last byte of the buffer is used for:
    // - "is small" bit - the highest bit - if "0" then it is small - otherwise
    // its "1" (128)
    // - if small - capacity left before going on the heap - using the lowest 5
    // bits
    // - if small - 2 bits are left unused - the second and third highest ones
    // - if small - acts as a null terminator if strlen() is 23 (24 including
    // the null terminator)
    ...
    // - push_back/pop_back
    // - assign/insert/erase
    // - relational operators as free functions - taking const char* as one of
    // the params
    class DOCTEST_INTERFACE String
    {
        static const unsigned len = 24; //! OCLINT avoid private static members
        static const unsigned last =
            len - 1; //! OCLINT avoid private static members

        struct view // len should be more than sizeof(view) - because of the
    ...
    public:
        ...
        // the only functions I'm willing to leave in the interface - available
        // for inlining
    ...
    namespace assertType
    {
        enum Enum
        {
            ...
            is_unary = 2 * is_false, // not checked anywhere - used just to
            ...
            DT_REQUIRE_THROWS_WITH_AS =
                is_throws_with | is_throws_as | is_require,
            ...
            DT_REQUIRE_UNARY_FALSE =
                is_normal | is_false | is_unary | is_require,
        };
    } // namespace assertType
    ...
    struct ContextOptions //! OCLINT too many fields
    {
        ...
        bool exit;        // if the program should be exited after the tests are
                          // ran/whatever
        bool duration;    // print the time duration of each test case
        bool no_throw;    // to skip exceptions-related assertion macros
        bool no_exitcode; // if the framework should return 0 as the exitcode
        bool no_run;     // to not run the tests at all (can be done with an "*"
                         // exclude)
        ...
        bool gnu_file_line; // if line numbers should be surrounded with :x: and
                            // not (x):
        bool no_path_in_filenames; // if the path to files should be removed
                                   // from the output
        bool no_line_numbers; // if source code line numbers should be omitted
                              // from the output
    ...
    namespace detail
    {
#if defined(DOCTEST_CONFIG_TREAT_CHAR_STAR_AS_STRING) ||                       \
    defined(DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS)
    ...
    template<class T> struct remove_const<const T> { typedef T type; };
        ...
        template <typename T> struct deferred_false
        ...
        namespace has_insertion_operator_impl
        {
            ...
            template <class, class = void> struct check
            {
        ...
        DOCTEST_INTERFACE String rawMemoryToString(const void *object,
                                                   unsigned size);
    ...
    template <typename T> String toString(const DOCTEST_REF_WRAP(T) value)
    {
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
        template <typename T>
        typename detail::enable_if<std::is_constructible<double, T>::value,
                                   Approx &>::type
...
#undef DOCTEST_APPROX_PREFIX
    ...
    template<class T> struct can_use_op : public not_char_pointer<typename decay_array<T>::type> {};
       ...
       // clang-format on
        ...
        String stringifyBinaryExpr(const DOCTEST_REF_WRAP(L) lhs,
                                   const char *op,
...
#define DOCTEST_FORBIT_EXPRESSION(rt, op)                                      \
    ...
    {                                                                          \
        ...
        // DOCTEST_MSVC_SUPPRESS_WARNING(4805) // 'operation' : unsafe mix of
        // type 'type' and type 'type' in operation
...
#define DOCTEST_CMP_LE(l, r) le(l, r)
        ...
        DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(<=, " <= ", DOCTEST_CMP_LE) //!OCLINT bitwise operator in conditional
            ...
            DOCTEST_FORBIT_EXPRESSION(Expression_lhs, ^=)
            DOCTEST_FORBIT_EXPRESSION(Expression_lhs, |=)
            // these 2 are unfortunate because they should be allowed - they
            // have higher precedence over the comparisons, but the
            // ExpressionDecomposer class uses the left shift operator to
            // capture the left operand of the binary expression...
            DOCTEST_FORBIT_EXPRESSION(Expression_lhs, <<)
            DOCTEST_FORBIT_EXPRESSION(Expression_lhs, >>)
        };
        ...
        struct DOCTEST_INTERFACE ExpressionDecomposer
        {
            ...
            // The right operator for capturing expressions is "<=" instead of
            // "<<" (based on the operator precedence table) but then there will
            // be warnings from GCC about "-Wparentheses" and since "_Pragma()"
            // is problematic this will stay for now...
            // https://github.com/catchorg/Catch2/issues/870
            // https://github.com/catchorg/Catch2/issues/565
            template <typename L>
            Expression_lhs<const DOCTEST_REF_WRAP(L)>
            operator<<(const DOCTEST_REF_WRAP(L) operand)
            {
        ...
        struct DOCTEST_INTERFACE TestCase : public TestCaseData
        {
            ...
            const char *m_type; // for templated test cases - gets appended to
                                // the real name
            int m_template_id;  // an ID used to distinguish between the
                                // different versions of a templated test case
...
#define DOCTEST_ASSERT_IN_TESTS(decomp)                                        \
    ...
    throwException()
                      ...
                      const DOCTEST_REF_WRAP(R) rhs)
        {
            ...
            // ###################################################################################
            // IF THE DEBUGGER BREAKS HERE - GO 1 LEVEL UP IN THE CALLSTACK FOR
            // THE FAILING ASSERT THIS IS THE EFFECT OF HAVING
                     ...
                     const char *expr, const DOCTEST_REF_WRAP(L) val)
        {
            ...
            if (at &
                assertType::is_false) //! OCLINT bitwise operator in conditional
                failed = !failed;
        ...
        public:
            ...
            bool translate(String &res) const override
            {
                ...
                { // NOLINT
                    res = m_translateFunction(
                        ex); //! OCLINT parameter reassignment
                    return true;
                }
                catch (...)
                {
        ...
        template <typename T> void toStream(std::ostream *s, const T &value)
        {
        ...
        DOCTEST_INTERFACE void toStream(std::ostream *s,
                                        int long long unsigned in);

        // ContextScope base class used to allow implementing methods of
        // ContextScope that don't depend on the template parameter in
        // doctest.cpp.
...
#define DOCTEST_DEFINE_DECORATOR(name, type, def)                              \
    ...
    int registerExceptionTranslator(String (*translateFunction)(T))
    {
        ...
        static detail::ExceptionTranslator<T> exceptionTranslator(
            translateFunction);
        DOCTEST_CLANG_SUPPRESS_WARNING_POP
...
namespace doctest
{
    ...
    template <typename T> int registerExceptionTranslator(String (*)(T))
    {
        return 0;
    }
    ...
    namespace TestCaseFailureReason
    {
        enum Enum
        {
            ...
            DidntFailExactlyNumTimes =
                128,                     // see the expected_failures decorator
    ...
    {
        // The constructor has to accept "const ContextOptions&" as a single
        // argument which has most of the options for the run + a pointer to the
        // stdout stream Reporter(const ContextOptions& in)
        ...
        // called whenever a subcase is entered (don't cache pointers to the
        // input)
        virtual void subcase_start(const SubcaseSignature &) = 0;
        // called whenever a subcase is exited (don't cache pointers to the
        // input)
        ...
        // filters, has a skip decorator or isn't in the execution range
        // (between first and last) (safe to cache a pointer to the input)
        virtual void test_case_skipped(const TestCaseData &) = 0;

        // doctest will not be managing the lifetimes of reporters given to it
        // but this would still be nice to have
        virtual ~IReporter();

        // can obtain all currently active contexts and stringify them if one
        // wishes to do so
        static int get_num_active_contexts();
        static const IContextScope *const *get_active_contexts();

        // can iterate through contexts which have been stringified
        // automatically in their destructors when an exception has been thrown
...
#define DOCTEST_TYPE_TO_STRING_IMPL(...)                                       \
    template <> inline const char *type_to_string<__VA_ARGS__>()               \
    {                                                                          \
        return "<" #__VA_ARGS__ ">";                                           \
    }
...
#define DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(id, anon, ...)             \
    DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_CAT(anon, DUMMY)) =                     \
        doctest::detail::instantiationHelper(DOCTEST_CAT(id, ITERATOR) <       \
                                             __VA_ARGS__ >                     \
                                             (__FILE__, __LINE__, 0));         \
    DOCTEST_GLOBAL_NO_WARNINGS_END()
...
#define DOCTEST_ASSERT_IMPLEMENT_2(assert_type, ...)                           \
    ...
    DOCTEST_WRAP_IN_TRY(                                                       \
        _DOCTEST_RB.setResult(doctest::detail::ExpressionDecomposer(           \
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR(signature)                       \
    template <typename DOCTEST_UNUSED_TEMPLATE_TYPE>                           \
    static inline doctest::String DOCTEST_ANONYMOUS(                           \
    ...
    namespace detail
    {
        ...
        String rawMemoryToString(const void *object, unsigned size)
        {
            ...
            unsigned const char *bytes =
                static_cast<unsigned const char *>(object);
            std::ostringstream oss;
        ...
        {
            g_oss.clear(); // there shouldn't be anything worth clearing in the
                           // flags
            g_oss.str(""); // the slow way of resetting a string stream
            // g_oss.seekp(0); // optimal reset - as seen here:
            // https://stackoverflow.com/a/624291/3162383
            return &g_oss;
        }

        String getTlsOssResult()
        {
            // g_oss << std::ends; // needed - as shown here:
            // https://stackoverflow.com/a/624291/3162383
        ...
        ticks_t getCurrentTicks()
        {
            ...
            return ((t.QuadPart - hzo.QuadPart) * LONGLONG(1000000)) /
                   hz.QuadPart;
        }
#else  // DOCTEST_PLATFORM_WINDOWS
        ticks_t getCurrentTicks()
        ...
        struct Timer
        {
            ...
            //    return static_cast<unsigned int>(getElapsedMicroseconds() /
            //    1000);
            //}
            double getElapsedSeconds() const
            {
                return static_cast<double>(getCurrentTicks() - m_ticks) /
                       1000000.0;
            }
        ...
        // this holds both parameters from the command line and runtime data for
        // tests
        struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
        {
            ...
            void finalizeTestCaseData()
            {
                ...
                if (Approx(currentTest->m_timeout).epsilon(DBL_EPSILON) != 0 &&
                    Approx(seconds).epsilon(DBL_EPSILON) >
                        currentTest->m_timeout)
                ...
                if (currentTest->m_should_fail)
                {
                    ...
                    {
                        failure_flags |=
                            TestCaseFailureReason::ShouldHaveFailedAndDid;
                    }
                    else
                    {
                        failure_flags |=
                ...
                {
                    failure_flags |=
                        TestCaseFailureReason::CouldHaveFailedAndDid;
                }
                else if (currentTest->m_expected_failures > 0)
                {
                    if (numAssertsFailedCurrentTest ==
                        currentTest->m_expected_failures)
                    {
                        failure_flags |=
                            TestCaseFailureReason::FailedExactlyNumTimes;
                    }
                    else
                    {
                        failure_flags |=
        ...
        // TODO: figure out if this is indeed necessary/correct - seems like
        // either there still could be a race or that there wouldn't be a race
        // even if using the context directly
        DOCTEST_THREAD_LOCAL bool g_no_colors;

#endif // DOCTEST_CONFIG_DISABLE
    }  // namespace detail

    void String::setOnHeap()
    {
...
const char* assertString(assertType::Enum at) {
    ...
    const char *failureString(assertType::Enum at)
    {
        if (at & assertType::is_warn) //! OCLINT bitwise operator in conditional
            return "WARNING";
        if (at & assertType::is_check) //! OCLINT bitwise operator in
                                       //! conditional
            return "ERROR";
        if (at &
...
namespace doctest
{
    namespace
    {
        // the int (priority) is part of the key for automatic sorting - sadly
        // one can register a reporter with a duplicate name and a different
        // priority but hopefully that won't happen often :|
...
#define DOCTEST_ITERATE_THROUGH_REPORTERS(function, ...)                       \
    ...
    curr_rep->function(__VA_ARGS__)
        ...
        {
            if (at & assertType::is_require) //! OCLINT bitwise operator in
                                             //! conditional
                return true;
    ...
    {
        using namespace detail;
        // matching of a string against a wildcard mask (case sensitivity
        // configurable) taken from
    ...
    namespace detail
    {
            ...
            : m_signature({name, file, line})
        {
            ...
            {
                // pop - revert to previous stack since we've already passed
                // this
        ...
        Subcase::~Subcase()
        {
            if (m_entered)
            {
                // only mark the subcase stack as passed if no subcases have
                // been skipped
    ...
    __cpp_lib_uncaught_exceptions >= 201411L
        ...
        {
            static_cast<TestCaseData &>(*this) =
                static_cast<const TestCaseData &>(other);
    ...
    namespace
    {
        ...
        {
            // this is needed because MSVC gives different case for drive
            // letters for __FILE__ when evaluated in a header and a source file
            const int res =
                lhs->m_file.compare(rhs->m_file, bool(DOCTEST_MSVC));
            if (res != 0)
                return res < 0;
...
#ifdef DOCTEST_CONFIG_COLORS_WINDOWS
            if (g_no_colors || (isatty(fileno(stdout)) == false &&
                                getContextOptions()->force_colors == false))
                return;

#define DOCTEST_SET_ATTR(x)                                                    \
    SetConsoleTextAttribute(g_stdoutHandle, x | g_origBgAttrs)
...
// clang-format on
    ...
    namespace detail
    {
        ...
        // destroy cannot be inlined into the destructor because that would mean
        // calling stringify after ContextScope has been destroyed (base class
        // destructors run after derived class destructors). Instead,
        // ContextScope calls this method directly from its destructor.
    ...
    !defined(DOCTEST_CONFIG_WINDOWS_SEH)
        ...
        struct FatalConditionHandler
        {
            ...
            handleException(PEXCEPTION_POINTERS ExceptionInfo)
            {
        ...
        LPTOP_LEVEL_EXCEPTION_FILTER FatalConditionHandler::previousTop =
            nullptr;
        ...
        struct FatalConditionHandler
        {
            ...
            static void reset()
            {
                if (isSet)
                {
                    ...
                    for (std::size_t i = 0; i < DOCTEST_COUNTOF(signalDefs);
                         ++i)
                    {
        ...
        bool FatalConditionHandler::isSet = false;
        struct sigaction FatalConditionHandler::oldSigActions[DOCTEST_COUNTOF(
            signalDefs)] = {};
...
#define DOCTEST_OUTPUT_DEBUG_STRING(                                           \
    ...
    namespace detail
    {
...
#if DOCTEST_MSVC
            if (m_expr[0] == ' ') // this happens when variadic macros are
                                  // disabled under MSVC
                ++m_expr;
#endif // MSVC
        }
        ...
        bool MessageBuilder::log()
        {
            ...
            // warn is just a message in this context so we don't treat it as an
            // assert
            if (!isWarn)
            {
            ...
            return isDebuggerActive() && !getContextOptions()->no_breaks &&
                   !isWarn; // break
        }

        void MessageBuilder::react()
        {
            if (m_severity & assertType::is_require) //! OCLINT bitwise operator
                                                     //! in conditional
                throwException();
        }
    ...
    namespace
    {
        ...
        DOCTEST_NORETURN void throw_exception(Ex const &e)
        {
            ...
            throw e;
#else  // DOCTEST_CONFIG_NO_EXCEPTIONS
            std::cerr << "doctest will terminate because it needed to throw an "
                         "exception.\n"
...
#define DOCTEST_INTERNAL_ERROR(msg)                                            \
    throw_exception(std::logic_error(__FILE__ ":" DOCTEST_TOSTR(               \
        __LINE__) ": Internal doctest error: " msg))
...
// =================================================================================================
        ...
        struct XmlReporter : public IReporter
        {
            ...
            // caching pointers/references to objects of these types - safe to
            // do
            ...
            void test_case_start_impl(const TestCaseData &in)
            {
                ...
                xml.startElement("TestCase")
                    ...
                    .writeAttribute("filename",
                                    skipPathFromFilename(in.m_file.c_str()))
            ...
            // =========================================================================================
            // WHAT FOLLOWS ARE OVERRIDES OF THE VIRTUAL METHODS OF THE REPORTER
            // INTERFACE
            // =========================================================================================
                    ...
                    for (unsigned i = 0; i < in.num_data; ++i)
                    {
                        xml.scopedElement("TestCase")
                            ...
                            .writeAttribute("testsuite",
                                            in.data[i]->m_test_suite)
                            .writeAttribute("filename",
                                            skipPathFromFilename(
                                                in.data[i]->m_file.c_str()))
            ...
            {
                // remove .exe extension - mainly to have the same output on
                // UNIX and Windows
                std::string binary_name =
                    skipPathFromFilename(opt.binary_name.c_str());
#ifdef DOCTEST_PLATFORM_WINDOWS
                if (binary_name.rfind(".exe") != std::string::npos)
                    binary_name =
                        binary_name.substr(0, binary_name.length() - 4);
#endif // DOCTEST_PLATFORM_WINDOWS

                xml.startElement("doctest").writeAttribute("binary",
                                                           binary_name);
            ...
            {
                if (tc) // the TestSuite tag - only if there has been at least 1
                        // test case
                    xml.endElement();

                xml.scopedElement("OverallResultsAsserts")
                    .writeAttribute("successes",
                                    p.numAsserts - p.numAssertsFailed)
                ...
                xml.startElement("OverallResultsAsserts")
                    .writeAttribute("successes",
                                    st.numAssertsCurrentTest -
                                        st.numAssertsFailedCurrentTest)
                    .writeAttribute("failures", st.numAssertsFailedCurrentTest);
                if (opt.duration)
                    xml.writeAttribute("duration", st.seconds);
                if (tc->m_expected_failures)
                    xml.writeAttribute("expected_failures",
                                       tc->m_expected_failures);
                xml.endElement();
            ...
            void log_assert(const AssertData &rb) override
            {
                ...
                if (rb.m_threw)
                    xml.scopedElement("Exception")
                        .writeText(rb.m_exception.c_str());

                if (rb.m_at & assertType::is_throws_as)
                    xml.scopedElement("ExpectedException")
                        .writeText(rb.m_exception_type);
                if (rb.m_at & assertType::is_throws_with)
                    xml.scopedElement("ExpectedExceptionString")
                        .writeText(rb.m_exception_string);
                if ((rb.m_at & assertType::is_normal) && !rb.m_threw)
                    xml.scopedElement("Expanded")
                        .writeText(rb.m_decomp.c_str());
        ...
        void fulltext_log_assert_to_stream(std::ostream &s,
                                           const AssertData &rb)
        {
            if ((rb.m_at &
                 (assertType::is_throws_as | assertType::is_throws_with)) ==
                0) //! OCLINT bitwise operator in conditional
            ...
            { //! OCLINT bitwise operator in conditional
                s << (rb.m_threw ? "threw as expected!"
            ...
            { //! OCLINT
                ...
                if (rb.m_threw)
                {
                    ...
                    {
                        s << "threw a DIFFERENT exception! (contents: "
                          << rb.m_exception << ")\n";
                    }
                }
                else
            ...
            { //! OCLINT bitwise operator in conditional
                s << (rb.m_threw ? "THREW exception: " : "didn't throw!")
                  << Color::Cyan << rb.m_exception << "\n";
            }
            else
            {
        ...
        struct JUnitReporter : public IReporter
        {
            ...
            {
                DOCTEST_CLANG_SUPPRESS_WARNING_WITH_PUSH(
                    "-Wdeprecated-declarations") // gmtime
                static std::string getCurrentTimestamp()
                {
                    // Beware, this is not reentrant because of backward
                    // compatibility issues Also, UTC only, again because of
                    // backward compatibility (%z is C++11)
                    time_t rawtime;
                    std::time(&rawtime);
                ...
                {
                    JUnitTestCase(const std::string &_classname,
                                  const std::string &_name)
                        : classname(_classname), name(_name), time(0),
                          failures()
                    {
                    }
                ...
                void
                appendSubcaseNamesToLastTestcase(std::vector<String> nameStack)
                {
                    for (auto &curr : nameStack)
                        if (curr.size())
                            testcases.back().name +=
                                std::string("/") + curr.c_str();
                }
            ...
            void test_run_end(const TestRunStats &p) override
            {
                ...
                for (const auto &testCase : testCaseData.testcases)
                {
                    ...
                    // This is not ideal, but it should be enough to mimic
                    // gtest's junit output.
            ...
            void test_case_start(const TestCaseData &in) override
            {
                testCaseData.add(skipPathFromFilename(in.m_file.c_str()),
                                 in.m_name);
                timer.start();
            }
            ...
            void test_case_reenter(const TestCaseData &in) override
            {
                ...
                testCaseData.appendSubcaseNamesToLastTestcase(
                    deepestSubcaseStackNames);
            ...
            void log_assert(const AssertData &rb) override
            {
                if (!rb.m_failed) // report only failures & ignore the `success`
                                  // option
                    return;
                ...
                os << skipPathFromFilename(rb.m_file)
                   << (opt.gnu_file_line ? ":" : "(") << line(rb.m_line)
        ...
        struct ConsoleReporter : public IReporter
        {
            ...
            // =========================================================================================
            // WHAT FOLLOWS ARE HELPERS USED BY THE OVERRIDES OF THE VIRTUAL
            ...
            const char *getSuccessOrFailString(bool success,
                                               assertType::Enum at,
            ...
            void logTestStart()
            {
                ...
                if (tc->m_description)
                    s << Color::Yellow << "DESCRIPTION: " << Color::None
                      << tc->m_description << "\n";
                if (tc->m_test_suite && tc->m_test_suite[0] != '\0')
                    s << Color::Yellow << "TEST SUITE: " << Color::None
                      << tc->m_test_suite << "\n";
                ...
                {
                    s << Color::Yellow
                      << "\nDEEPEST SUBCASE STACK REACHED (DIFFERENT FROM THE "
                         "CURRENT ONE):\n"
                      << Color::None;
            ...
            {
                printVersion();
                auto printReporters = [this](const reporterMap &reporters,
                                             const char *type) {
                    if (reporters.size())
                    {
                        s << Color::Cyan << "[doctest] " << Color::None
            ...
            void list_query_results()
            {
                ...
                else if (opt.list_test_suites)
                {
                    s << Color::Cyan << "[doctest] " << Color::None
                      ...
                      << g_cs->numTestCasesPassingFilters << "\n";
                    s << Color::Cyan << "[doctest] " << Color::None
                      << "test suites with unskipped test cases passing the "
                         "current filters: "
            ...
            void test_run_end(const TestRunStats &p) override
            {
                ...
                s << Color::Cyan << "[doctest] " << Color::None
                  ...
                  << std::setw(6) << p.numTestCasesFailed << " failed"
                  << Color::None << " | ";
                if (opt.no_skipped_summary == false)
                {
                    const int numSkipped =
                        p.numTestCases - p.numTestCasesPassingFilters;
                    s << (numSkipped == 0 ? Color::None : Color::Yellow)
                      << std::setw(6) << numSkipped << " skipped"
                      << Color::None;
                }
                s << "\n";
                s << Color::Cyan << "[doctest] " << Color::None
                  << "assertions: " << std::setw(6) << p.numAsserts << " | "
                  << ((p.numAsserts == 0 || anythingFailed) ? Color::None
                                                            : Color::Green)
                  << std::setw(6) << (p.numAsserts - p.numAssertsFailed)
                  << " passed" << Color::None << " | "
                  << (p.numAssertsFailed > 0 ? Color::Red : Color::None)
                  << std::setw(6) << p.numAssertsFailed << " failed"
                  << Color::None << " |\n";
                s << Color::Cyan << "[doctest] " << Color::None << "Status: "
                  << (p.numTestCasesFailed > 0 ? Color::Red : Color::Green)
                  << ((p.numTestCasesFailed > 0) ? "FAILURE!" : "SUCCESS!")
                  << Color::None << std::endl;
            }
            ...
            void test_case_end(const CurrentTestCaseStats &st) override
            {
                ...
                // else to print - something other than that an assert has
                // failed
                if (opt.duration ||
                    (st.failure_flags &&
                ...
                if (opt.duration)
                    s << Color::None << std::setprecision(6) << std::fixed
                ...
                {
                    s << Color::Red
                      << "Should have failed but didn't! Marking it as "
                         "failed!\n";
                }
                         ...
                         TestCaseFailureReason::CouldHaveFailedAndDid)
                {
                    s << Color::Yellow
                      << "Allowed to fail so marking it as not failed\n";
                         ...
                         TestCaseFailureReason::DidntFailExactlyNumTimes)
                {
                    s << Color::Red << "Didn't fail exactly "
                      << tc->m_expected_failures
                      << " times so marking it as failed!\n";
                         ...
                         TestCaseFailureReason::FailedExactlyNumTimes)
                {
                    s << Color::Yellow << "Failed exactly "
                      << tc->m_expected_failures
                      << " times as expected so marking it as not failed!\n";
                }
                if (st.failure_flags &
                    TestCaseFailureReason::TooManyFailedAsserts)
                {
            ...
            void test_case_exception(const TestCaseException &e) override
            {
                ...
                successOrFailColoredStringToStream(
                    false,
                    e.is_crash ? assertType::is_require : assertType::is_check);
                s << Color::Red
    ...
    {                                                                          \
                             ...
                             const char *pattern, String *value)
        {
            ...
            for (int i = argc; i > 0; --i)
            {
                ...
                { //! OCLINT prefer early exits and continue
                    // eliminate matches in which the chars before the option
                    // are not '-'
                               ...
                               const char *pattern, std::vector<String> &res)
        {
            ...
            if (parseOption(argc, argv, pattern, &filtersString))
            {
                ...
                auto pch = std::strtok(filtersString.c_str(),
                                       ","); // modifies the string
                while (pch != nullptr)
                {
                    if (strlen(pch))
                            ...
                            const char *pattern, optionType type, int &res)
        {
            ...
            {
                // integer
                // TODO: change this to use std::stoi or something else!
                // currently it uses undefined behavior - assumes '0' on failed
                // parse...
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                    \
    ...
    // allows the user to override procedurally the int/bool options from the
    // command line
    ...
    // allows the user to override procedurally the string options from the
    // command line
    ...
    int Context::run()
    {
        ...
        // save the old context state in case such was setup - for using asserts
        // out of a testing context
        auto old_cs = g_cs;
        ...
        auto cleanup_and_return = [&]() {
            ...
            // we have to free the reporters which were allocated when the run
            // started
        ...
        // invoke the registered functions if they match the filter criteria (or
        // just count them)
        for (auto &curr : testArray)
        {
            ...
            if ((p->last < p->numTestCasesPassingFilters &&
                 p->first <= p->last) ||
            ...
            // do not execute the test if we are to only count the number of
            ...
            // execute the test if it passes all the filtering
            {
                ...
                do
                {
                    // reset some of the fields for subcases (except for the set
                    // of fully passed ones)
...
#endif // DOCTEST_CONFIG_NO_EXCEPTIONS
                        FatalConditionHandler
                            fatalConditionHandler; // Handle signals
                        // execute the test
                        tc.m_test();
                        fatalConditionHandler.reset();
#ifndef DOCTEST_CONFIG_NO_EXCEPTIONS
                    ...
                    {
                        p->failure_flags |=
                            TestCaseFailureReason::AssertFailure;
                    }
                    catch (...)
                    {
                        DOCTEST_ITERATE_THROUGH_REPORTERS(
                    ...
                    // exit this loop if enough assertions have failed - even if
                    // there are more subcases
                    if (p->abort_after > 0 &&
                        p->numAssertsFailed +
                                p->numAssertsFailedCurrentTest_atomic >=
                            p->abort_after)
                    {
                        run_test = false;
                        p->failure_flags |=
                    ...
                    if (p->should_reenter && run_test)
                        DOCTEST_ITERATE_THROUGH_REPORTERS(test_case_reenter,
                                                          tc);
                    if (!p->should_reenter)
                        run_test = false;
                } while (run_test);
    ...
    const String *IReporter::get_stringified_contexts()
    {
        return get_num_stringified_contexts()
                   ? &detail::g_cs->stringifiedContexts[0]
                   : nullptr;
    }
                                  ...
                                  reporterCreatorFunc c, bool isReporter)
        {
            if (isReporter)
                getReporters().insert(reporterMap::value_type(
                    reporterMap::key_type(priority, name), c));
            else
                getListeners().insert(reporterMap::value_type(
                    reporterMap::key_type(priority, name), c));
        }
    } // namespace detail
