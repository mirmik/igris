...
namespace igris
{
	...
	public:
			...
			result<void> check(const trent& tr, igris::strvec& strvec) const
			{
				switch (type)
				{
					...
					case list_checker_type:
						if (!tr.is_list()) return error(nos::format("trent {} should be list", strvec[strvec.size() - 1]));
						if (len != -1 && tr.as_list().size() != (unsigned int)len) return error(nos::format("array size should be {}", len));
						break;
				...
				if (type == dict_checker_type)
				{
					...
					if (_inset)
					{
						for (auto& tp : tr.unsafe_dict_const())
						{
							...
							if (!flag) return error(nos::format("subopt {} not in validation set", tp.first));
						}
					}
				...
				if (type == dict_checker_type && check_dict)
				{
					...
					for (const auto& n : nodes)
					{
						...
						if (!tr.have(n.first))
						{
							//igris::println("check_dict doesn't have", strvec);
							if (n.second._optional == false)
							{
								if (!_ifexist) return error(nos::format("subopt {} isn't exist", n.first));
							}
							else
							{
								//igris::fprintln("don't have optional node {}", strvec);
							}
						}
						else
						{
					...
					if (_allopts)
					{
						for (auto& tp : tr.unsafe_dict_const())
						{
							if (nodes.count(tp.first) == 0)  return error(nos::format("unresolved options {}", tp.first));
						}
					}
				}
				...
				if (type == string_checker_type && _inset)
				{
					...
					if (!flag) return error(nos::format("{} not in validation set", tr.unsafe_string_const()));
				}
		...
		void asserted_check(const trent& tr, std::string rootname)
		{
			...
			if (ret.is_error())
			{
				std::string path = igris::join(strvec, '/');
				std::string errtxt = nos::format("SCHEMA: trent {} {}", path, ret.error().what());
