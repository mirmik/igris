...
void stimer_init(struct stimer_head * timer, 
	unsigned long start, long interval);

// Инициализировать и активировать таймер
void stimer_plan(struct stimer_head * timer, 
	unsigned long start, long interval);

// Сменить точку старта и запланировать
void stimer_start(struct stimer_head * timer, 
	unsigned long start);

// Проверить, сработал ли таймер
int stimer_check(struct stimer_head * timer, 
