...
namespace gxx {
	...
	std::string base64_decode(std::string const& encoded_string) {
		...
		while (in_len-- && ( encoded_string[in_] != '=') && is_base64(encoded_string[in_])) {
			...
			if (i ==4) {
				for (i = 0; i <4; i++)
					char_array_4[i] = (unsigned char)(strchr(base64_charset, char_array_4[i]) - base64_charset);
		...
		if (i) {
			...
			for (j = 0; j <4; j++)
					char_array_4[j] = (unsigned char)(strchr(base64_charset, char_array_4[j]) - base64_charset);
