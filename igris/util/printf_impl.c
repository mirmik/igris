...
#define OPS_FLAG_WITH_SPEC                                                     \
    0x00000008 /* print a prefix for non-decimal systems */
#define OPS_FLAG_ZERO_PAD 0x00000010 /* padding with zeroes */
#define OPS_PREC_IS_GIVEN 0x00000020 /* precision is specified */
#define OPS_LEN_MIN                                                            \
    0x00000040 /* s_char (d, i); u_char (u, o, x, X); s_char* (n) */
#define OPS_LEN_SHORT                                                          \
    0x00000080 /* short (d, i); u_short (u, o, x, X); short* (n) */
#define OPS_LEN_LONG                                                           \
    0x00000100 /* long (d, i); u_long (u, o, x, X); wint_t (c); wchar_t(s);    \
                  long* (n) */
#define OPS_LEN_LONGLONG                                                       \
    0x00000200 /* llong (d, i); u_llong (u, o, x, X); llong* (n) */
#define OPS_LEN_MAX                                                            \
    0x00000400 /* intmax_t (d, i); uintmax_t (u, o, x, X); intmax_t* (n) */
#define OPS_LEN_SIZE 0x00000800 /* size_t (d, i, u, o, x, X); size_t* (n) */
#define OPS_LEN_PTRDIFF                                                        \
    0x00001000 /* ptrdiff_t (d, i, u, o, x, X); ptrdiff_t* (n) */
 ...
 * Options for print_i
 */
#define PRINT_I_BUFF_SZ                                                        \
    23 /* size of buffer for long long int (64bit) -- that's enough for oct,   \
          dec and hex base systems */

/**
 * Options for print_f
 */
#define PRINT_F_BUFF_SZ                                                        \
    65 /* size of buffer for long double -- FIXME this may not be enough */
                   ...
                   unsigned int ops)
{
    ...
    if (!(ops & OPS_FLAG_LEFT_ALIGN))
    {
        ...
        for (; space_count; --space_count)
            printchar_handler(printchar_data, ' ');
    }

    pc += len;
    while (len--)
        printchar_handler(printchar_data, *str++);
                   ...
                   int base)
{
    ...
    *end = '\0';
    prefix = is_signed && ((long long int)u < 0)         ? (u = -((int)u), "-")
    ...
    pc += prefix_len;
    while (prefix_len--)
        printchar_handler(printchar_data, *prefix++);

    pc += zero_count;
    while (zero_count--)
        printchar_handler(printchar_data, '0');
                   ...
                   int is_shortened)
{
    ...
    *end = '\0';
    ...
    precision -= is_shortened ? (int)ceill(LOG10(ip)) + (ip != 0.0L) : 0;
    for (; (sign_count < precision) && ((int)FMOD(fp, 1.0L) != 0.0L);
         ++sign_count)
        fp *= base;
    fp = roundl(fp);

    ip = precision ? fp != POW(base, sign_count) ? ip : ip + 1.0L
                   : roundl(ip + fp);
    ...
    len = (int)(end - str);
    postfix_len = (int)strlen(postfix);
    ...
    pc += postfix_len;
    while (postfix_len--)
        printchar_handler(printchar_data, *postfix++);
             ...
             va_list args)
{
    ...
    after_flags:
        ...
        case 'l':
            ops |=
                *++format != 'l' ? OPS_LEN_LONG : (++format, OPS_LEN_LONGLONG);
            break;
        case 'j':
            ops |= OPS_LEN_MAX;
            ++format;
        ...
        case 'X':
            tmp.ulli =
                ...
                : ops & OPS_LEN_PTRDIFF
                    ? (unsigned long long)va_arg(args, ptrdiff_t)
