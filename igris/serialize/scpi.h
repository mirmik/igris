	...
	public:
		struct header {
			...
			size_t print_to(nos::ostream& o) const {
				if (num == -1) return nos::print(str);
				else return nos::fprint_to(o, "{}{}", str, num);
			}
		};
	...
	public:
		scpi_string_parser(const std::string& str) {
			...
			while(!reader.next_is("\0?"_b)) {
				if (reader.next_is('\"')) {
					reader.skip();
					arguments.emplace_back(reader.string_while(igris::chars_set_checker("\"\0"_b, false)));
					if (reader.next_is('\0')) return;
					else reader.skip();
				} else {
					arguments.emplace_back(reader.string_while(igris::chars_set_checker(" ,\0\n?"_b, false)));
				}
				reader.skip_while(" ,\n");
			}
