...
namespace igris
{
	...
	public:
		...
		{
			if (i >= (int)size() || i < 0) return false;
		...
		public:
			...
			unlinked_iterator next()
			{
				...
				while (_num < (int)_pool->size() && !_pool->cell_is_allocated(_num));
			...
			unlinked_iterator operator++() { *this = next(); return *this; }
			void* operator*() { return _pool->cell(_num); }
		};

		unlinked_iterator begin() const { return unlinked_iterator{this, -1} .next(); };
